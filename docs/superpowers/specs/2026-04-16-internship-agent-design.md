...
│
├── data/
│   └── raw/                 # kalibrr.json, jobstreet.json, internph.json
│
└── output/
...
Three scrapers, one per source:

| Scraper | Source | Notes |
|---|---|---|
| `internph.py` | InternPH | Simpler HTML, good starting point |
| `kalibrr.py` | Kalibrr | JS-heavy, may need Playwright fallback |
| `jobstreet.py` | JobStreet PH | JS-heavy, may need Playwright fallback |
...
| `deduplicate` | Merge | Detects duplicates by fuzzy match on company name + title keywords. Keeps the version with the most non-null fields; uses longest description as tiebreaker. Source priority if still tied: InternPH > Kalibrr > JobStreet |
...
| Scraping | requests + BeautifulSoup (Playwright as fallback for JS-heavy sites) |
